...
public class BonusExpManager
{
	...
	private void load()
	{
		try
		{
			...
			File file = new File(Config.DATAPACK_ROOT + "/data/stats/runas/BonusExpItems.xml");
			if (!file.exists())
			{
				...
				for (Node n = first.getFirstChild(); n != null; n = n.getNextSibling())
				{
					if ("bonus".equalsIgnoreCase(n.getNodeName()))
					{
						for (Node cd = n.getFirstChild(); cd != null; cd = cd.getNextSibling())
						{
							...
							{
								Node att = cd.getAttributes().getNamedItem("id");
								if (att != null)
								{
	...
	public long[] getBonusExpAndSp(L2PcInstance player, long exp, long sp)
	{
		...
		if (player != null)
		{
			...
			return new long[]
			        {
		            (long) ((exp * bonusExp) / 100.0D),
		            (long) ((sp * bonusSp) / 100.0D)
		        };
		}
		return new long[]
		{
