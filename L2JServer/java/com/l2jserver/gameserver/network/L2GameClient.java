		...
		public void run()
		{
			...
			try
			{
				...
				if (player != null && !isDetached())
				{
					...
					{
						if ((((player.getPrivateStoreType() == L2PcInstance.STORE_PRIVATE_SELL) || (player.getPrivateStoreType() == L2PcInstance.STORE_PRIVATE_BUY)) && Config.OFFLINE_TRADE_ENABLE) || ((player.isInCraftMode() || (player.getPrivateStoreType() == L2PcInstance.STORE_PRIVATE_MANUFACTURE)) && Config.OFFLINE_CRAFT_ENABLE || player.isSellBuff() && Config.OFFLINE_SELLBUFF_ENABLED))
						{
							player.leaveParty();
