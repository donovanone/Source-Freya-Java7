...
public final class Say2 extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (Config.BOT_DETECT && _type == TELL && checkBot(_text))
		{
			Util.handleIllegalPlayerAction(activeChar, "Client Emulator Detect: Player " + activeChar.getName() + " using l2walker.", Config.BOT_PUNISH);
			return;
		}
        ...
        if ((activeChar.getLevel() < 40) && (_type == TRADE || _type == SHOUT || _type == HERO_VOICE))
        {
            activeChar.sendMessage("Shout, trade and hero chatting cannot be used until level 40.");
            return;
        }
