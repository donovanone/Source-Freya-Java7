...
public class ExShowSellCropList extends L2GameServerPacket
{
	...
	public void writeImpl()
	{
		...
		for (L2ItemInstance item : _cropsItems.values())
		{
			...
			writeD(L2Manor.getInstance().getSeedLevelByCrop(item.getId())); // seed level
			writeC(1);
			writeD(L2Manor.getInstance().getRewardItem(item.getId(), 1)); // reward 1 id
			writeC(1);
			writeD(L2Manor.getInstance().getRewardItem(item.getId(), 2)); // reward 2 id
