...
public class EventPlayer implements Comparable<EventPlayer>
{
	...
	/** Cancelamos el casting de cualquier skill */
	public void abortCast()
	{
		_owner.abortCast();
	}
	
	/** Cancelamos cualquier tipo de ataque */
	...
	/**
	 * Verificamos si el player esta casteando
	 * @return
	 */
	...
	/**
	 * Verificamos si el player esta atacando
	 * @return
	 */
	public boolean isAttackingNow()
	{
		return _owner.isAttackingNow();
	}
	...
	public void removeFromParty()
	{
		if (_owner.getParty() != null)
		{
			...
			party.removePartyMember(_owner);
		}
	}
	...
	public void scorebarPacket(String text) {
		_owner.sendPacket(new ExShowScreenMessage(1, -1, 2, 0, 1, 0, 0, false,
				2000, 0, text));
	}
	...
	public void showEscapeEffect() {
		...
		_owner.forceIsCasting(GameTimeController.getGameTicks() + 10000
	...
	public void teleport(PLoc loc, int delay, boolean randomOffset, int instanceId)
	{
		...
		if (summon != null)
		{
			summon.stopAllEffects(); // posible fix
	...
	{
		L2ItemInstance wpn = _owner.getInventory().getPaperdollItem(Inventory.PAPERDOLL_RHAND);
		// Fix
		if (wpn != null)
		{
			_owner.getInventory().unEquipItemInBodySlotAndRecord(Inventory.PAPERDOLL_RHAND);
		}
		// Minor fix by Fissban
		wpn = _owner.getInventory().getPaperdollItem(Inventory.PAPERDOLL_LHAND);
		if (wpn != null)
		{
			_owner.getInventory().unEquipItemInBodySlotAndRecord(Inventory.PAPERDOLL_LHAND);
		}
