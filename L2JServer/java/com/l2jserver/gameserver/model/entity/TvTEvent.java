...
public class TvTEvent
{
				...
				if (!payParticipationFee(player))
					iter.remove();
					   ...
					   if (Config.ALLOW_AIO_BLOCK_EVENT)
						   ...
						   if(player.isAio())
						   {
								   player.sendMessage("Proibido uso de AIO em Eventos!");
								   return false;
						   }
				   }
	...
	if (Config.TVT_FRAGS_COUNTER)
	playerInstance._tvtpoints = 0;
				}
			}
		}
	...
	if (playerInstance._tvtpoints < Config.TVT_FRAGS_MIN_FOR_REWARD && Config.TVT_FRAGS_COUNTER)
	{
	systemMessage = new SystemMessage(SystemMessageId.NO_FUNDS_DUE);
	...
	//extraRewards
	if (playerInstance._tvtpoints >= Config.TVT_FRAGS_MIN_FOR_EXTRA_REWARD && Config.TVT_EXTRA_REWARD_FOR_FRAGS)
	{
	for (int[] extraReward : Config.TVT_EXTRA_REWARDS)
	{
	PcInventory inv = playerInstance.getInventory();
	int extraRewardByStep = 0;
	extraRewardByStep = Math.abs(playerInstance._tvtpoints / Config.TVT_FRAGS_FOR_REWARD_STEP);
	// Check for stackable item, non stackabe items need to be added one by one
	if (ItemTable.getInstance().createDummyItem(extraReward[0]).isStackable())
	{
	inv.addItem("TvT Event", extraReward[0], extraReward[1] * extraRewardByStep, playerInstance, playerInstance);
	
	if (extraReward[1] > 1)
	{
	systemMessage = new SystemMessage(SystemMessageId.EARNED_S2_S1_S);
	systemMessage.addItemName(extraReward[0]);
	systemMessage.addItemNumber(extraReward[1] * extraRewardByStep);
	}
	else
	{
	...
	{
	for (int i = 0; i < extraReward[1] * extraRewardByStep; ++i)
	{
	inv.addItem("TvT Event", extraReward[0], 1, playerInstance, playerInstance);
	systemMessage = new SystemMessage(SystemMessageId.EARNED_ITEM);
	systemMessage.addItemName(extraReward[0]);
	playerInstance.sendPacket(systemMessage);
	}
	...
	if (Config.TVT_FRAGS_COUNTER)
	killerPlayerInstance._tvtpoints += 1;
