...
abstract public class OlympiadGameNormal extends AbstractOlympiadGame
{
	...
	protected final boolean portPlayersToArena(List<Location> spawns)
	{
		...
		try
		{
			...
			int SegundosApareceraEnPantalla = 10;

			ExShowScreenMessage message = new ExShowScreenMessage("Your Opponent is:"  + CharTemplateTable.getInstance().getClassNameById(_playerTwo.player.getClassId().getId()),SegundosApareceraEnPantalla * 5000);
			_playerOne.player.sendPacket(message);
			message = new ExShowScreenMessage("Your Opponent is:"  + CharTemplateTable.getInstance().getClassNameById(_playerOne.player.getClassId().getId()),SegundosApareceraEnPantalla * 5000);
			_playerTwo.player.sendPacket(message);
			
			_playerOne.player.sendMessage("Your Opponent is:" + CharTemplateTable.getInstance().getClassNameById(_playerTwo.player.getClassId().getId()));
			_playerTwo.player.sendMessage("Your Opponent is:" + CharTemplateTable.getInstance().getClassNameById(_playerOne.player.getClassId().getId()));
		}
		catch (Exception e)
		{
