...
private boolean _isBuffProtected = false;
	...
	public static final byte NO_RESTART= 22;
        public static final byte ZONE_MULTIFUNCTION = 23;
        public static final byte ZONE_CLANWAR = 24;
 ...
 * @param d
		...
		else
		{
			...
			if(this instanceof L2PcInstance && target instanceof L2PcInstance)
				if(EventsInterface.isParticipating(getObjectId()))
					EventsInterface.onHit(getObjectId(), target.getObjectId());
	...
	{
		return Config.BUFFS_MAX_AMOUNT + Math.max(0, getSkillLevel(L2Skill.SKILL_DIVINE_INSPIRATION1)) +  Math.max(0, getSkillLevel(L2Skill.SKILL_DIVINE_INSPIRATION));
	}
	 ...
	 * @param state
	 */
	public void setInsideZone(ZoneId altered, boolean state)
	{
