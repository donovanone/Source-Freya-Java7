	...
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		if (Config.L2JMOD_CHAMPION_ENABLE && isChampion())
			dropChance *= Config.L2JMOD_CHAMPION_REWARDS;
		           ...
		           if (drop.getItemId() == Config.VIP_DROP_ITEM_ID && lastAttacker.isVip())
		...
		if (Config.L2JMOD_CHAMPION_ENABLE)
			...
			if ((drop.getItemId() == 57 || (drop.getItemId() >= 6360 && drop.getItemId() <= 6362)) && isChampion())
				itemCount *= Config.L2JMOD_CHAMPION_ADENAS_REWARDS;
		           ...
		           if (drop.getItemId() == Config.VIP_DROP_ITEM_ID && lastAttacker.isVip())
		                 itemCount *= Config.RATE_DROP_VIP;
		...
		if (Config.L2JMOD_CHAMPION_ENABLE && isChampion())
			categoryDropChance *= Config.L2JMOD_CHAMPION_REWARDS;
		       ...
		       if (lastAttacker.isVip())
			             categoryDropChance *= Config.RATE_DROP_VIP;
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character mainDamageDealer)
	{
		...
		if (Config.L2JMOD_CHAMPION_ENABLE && isChampion() && (Config.L2JMOD_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE > 0 || Config.L2JMOD_CHAMPION_REWARD_HIGHER_LVL_ITEM_CHANCE > 0))
		{
			...
			if (player.getLevel() <= getLevel() && (Rnd.get(100) < Config.L2JMOD_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE))
			{
	         ...
	         // Apply Special Item drop with rnd qty for vip player
	         if (player.isVip() && Config.VIP_DROP_ITEM_REWARD > 0 && (Rnd.get(100) < Config.VIP_DROP_ITEM_REWARD))
	         {
	             int vipqty = Rnd.get(Config.VIP_DROP_ITEM_QTY);
	                vipqty++; //quantity should actually vary between 1 and whatever admin specified as max, inclusive.
	
	                RewardItem item1 = new RewardItem(Config.VIP_DROP_ITEM_ID,vipqty);
	             if (Config.AUTO_LOOT) player.addItem("VipLoot", item1.getItemId(), item1.getCount(), this, true); // Give this or these Item(s) to the L2PcInstance that has killed the L2Attackable
	  else dropItem(player, item1);
	         }
