...
_fakePc = FakePcsTable.getInstance().getFakePc(template.npcId);
	...
	public boolean doDie(L2Character killer)
	{
				...
				if(player.getKills() == Config.ANTIBOT_KILL_MOBS)
				{
				...
				player.setCode(finalString);
				ThreadPoolManager.getInstance().scheduleGeneral(new CaptchaTimer(player), 180000);//180sec
				player.setCodeRight(false);
