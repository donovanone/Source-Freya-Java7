...
import handlers.admincommandhandlers.AdminRideTarget;
import handlers.admincommandhandlers.AdminMensaje;
import handlers.admincommandhandlers.AdminSitDeport;
...
import handlers.bypasshandlers.EventPhoenix;
...
import handlers.voicedcommandhandlers.CombineTalism;
...
import handlers.voicedcommandhandlers.hero;
import handlers.voicedcommandhandlers.delevel;
import handlers.voicedcommandhandlers.L2Tower;
...
import handlers.voicedcommandhandlers.VotingReward;
import handlers.voicedcommandhandlers.ClanMensaje;
...
import handlers.voicedcommandhandlers.Teleport1;
import handlers.voicedcommandhandlers.SetVCmd;
import handlers.voicedcommandhandlers.Premium;
import handlers.voicedcommandhandlers.PartyControlVC;
import handlers.voicedcommandhandlers.Event;
...
AdminCommandHandler.getInstance().registerAdminCommandHandler(new AdminRideTarget());
AdminCommandHandler.getInstance().registerAdminCommandHandler(new AdminMensaje());
AdminCommandHandler.getInstance().registerAdminCommandHandler(new AdminSitDeport());
 ...
 AdminCommandHandler.getInstance().registerAdminCommandHandler(new AdminClanFull());
		...
		AdminCommandHandler.getInstance().registerAdminCommandHandler(new AdminDanceSystem());
...
AdminCommandHandler.getInstance().registerAdminCommandHandler(new AdminMassHero());
...
AdminCommandHandler.getInstance().registerAdminCommandHandler(new AdminRecallAll());
	...
	private static void loadBypassHandlers()
	{
		...
		BypassHandler.getInstance().registerBypassHandler(new Leaderboard());
 ...
 BypassHandler.getInstance().registerBypassHandler(new DropInfo());
  BypassHandler.getInstance().registerBypassHandler(new EventPhoenix());
	...
	private static void loadVoicedHandlers()
	{
            ...
            VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new hero());
        if (Config.SHOW_USER)
            VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new User());      
        if (Config.SHOW_USER)
            VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new UserActions());
		...
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new L2Tower());
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new Service());
		...
		if (Config.VOTE_INDIVIDUAL)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new VotingReward());
		if (Config.COMBINETALISMANS)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new CombineTalism());
		if (Config.CHANGENAME)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new ChangeName());
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new stats());
		if (Config.CLANMENSAJE)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new ClanMensaje());
		if (Config.RES)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new Res());
		 if (Config.BOSS_STATS)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new RaidBoosStatus());
		 if (Config.DESCONECTAR)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new Logout());
		 if (Config.SIEGE)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new Siege());
		 if (Config.ALLOW_TELEPORT_VOICECOMMAND)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new Teleport());
		 if (Config.ALLOW_TELEPORT_VOICECOMMAND)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new Teleport1());
		 if (Config.CLAN_SET)
		VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new SetVCmd());
		if (Config.ALLOW_PARTY_CONTROL)
			VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new PartyControlVC()); 
 ...
 if (Config.REC_BUY)
 VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new BuyRec());
 if (Config.ALLOW_EXP_GAIN_COMMAND)
 VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new ExpGain());
 if (Config.COMMAND_ONLINE)
 VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new Online());
 if (Config.GO_LIDER_CLAN)
 VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new GoToClanLeader());
 if (Config.TOMOS)
 VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new OpenAtod());
       ...
       VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new VisualArmor());
              if (Config.DELEVEL)
            VoicedCommandHandler.getInstance().registerVoicedCommandHandler(new delevel());
