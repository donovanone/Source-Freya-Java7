...
public class Frintezza extends L2AttackableAIScript
{
	...
	public String onAdvEvent (String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			_Zone.broadcastPacket(new SocialAction(strongScarlet.getObjectId(), 1));
			L2Skill skill = SkillTable.getInstance().getInfo(5017, 1);
			if (skill != null)
		...
		{
			_Zone.broadcastPacket(new NpcSay(npc.getObjectId(),1,npc.getId(),"Exceeded his time limit, challenge failed!"));
		...
		else if (event.equalsIgnoreCase("spawn_minion"))
		{
			...
			{
				L2Attackable mob = (L2Attackable)addSpawn(npc.getId()+2,npc.getX(),npc.getY(),npc.getZ(),npc.getHeading(),false,0);
