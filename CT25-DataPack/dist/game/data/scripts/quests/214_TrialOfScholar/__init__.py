...
STRONG_LIQUOR_ID,CRYSTAL_OF_PURITY2_ID,CASIANS_LIST_ID,StaryS_SKIN_ID,MEDUSAS_BLOOD_ID, \
...
class Quest (JQuest) :
 ...
 def onTalk (self,npc,player):
   ...
   if npcId == 30461 and cond == 0 :
     if st.getInt("onlyone") == 0 :
        ...
        if st.getQuestItemsCount(StaryS_SKIN_ID)+st.getQuestItemsCount(MEDUSAS_BLOOD_ID)+st.getQuestItemsCount(FETTEREDSOULS_ICHOR_ID)+st.getQuestItemsCount(ENCHT_GARGOYLES_NAIL_ID)<32 :
 ...
 def onKill(self,npc,player,isPet):
   ...
   if st.getInt("cond") == 16 :
    ...
    if st.getInt("cond") == 28  and st.getQuestItemsCount(StaryS_SKIN_ID)<10 :
      st.giveItems(StaryS_SKIN_ID,1)
      if st.getQuestItemsCount(StaryS_SKIN_ID) < 10 :
        st.playSound("ItemSound.quest_itemget")
      else:
        st.playSound("ItemSound.quest_middle")
        if st.getQuestItemsCount(StaryS_SKIN_ID) == 10 and st.getQuestItemsCount(MEDUSAS_BLOOD_ID) == 12 and st.getQuestItemsCount(FETTEREDSOULS_ICHOR_ID) == 5 and st.getQuestItemsCount(ENCHT_GARGOYLES_NAIL_ID) == 5:
           st.set("cond","29")
   elif npcId == 20158 :
