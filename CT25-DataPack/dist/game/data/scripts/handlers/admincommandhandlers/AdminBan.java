	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_ban_acc"))
		{
			...
			else
			{
				...
				Announcements.getInstance().announceToAll("Account "+targetPlayer.getAccountName()+" banned.");
		...
		else if (command.startsWith("admin_jail"))
		{
			...
			if (targetPlayer != null)
			{
				...
				Announcements.getInstance().announceToAll("Character "+targetPlayer.getName()+" jailed for "+(duration>0 ? duration+" minutes." : "ever!"));
				auditAction(command, activeChar, targetPlayer.getName());
			}
			else
			{
